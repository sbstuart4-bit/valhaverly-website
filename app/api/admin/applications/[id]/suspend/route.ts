...
import { suspendApplication } from '@/lib/partner/admin-actions';
...
export async function POST(
  ...
  try {
    ...
    const updated = await suspendApplication(app, reason, admin.email);
    const saved = await saveApplication(updated);

    let emailResult = null;
    if (sendNotification && saved.personal.email) {
      const mail = buildPartnerEmail('suspended', {
    ...
    return errorResponse(err instanceof Error ? err.message : 'Suspension failed.', 500);
  }
}
